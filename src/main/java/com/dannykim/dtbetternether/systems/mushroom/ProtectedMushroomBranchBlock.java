...
public final class ProtectedMushroomBranchBlock extends MushroomBranchBlock {
    public ProtectedMushroomBranchBlock(final ResourceLocation name, final Properties properties) {
        super(name, properties);
    }

    @Override
    public void destroyMushroomCap(final Level level, final BlockPos cutPos, final Species species,
                                   final ItemStack tool, final List<BlockPos> endPoints,
                                   final Map<BlockPos, BlockState> destroyedCaps,
        ...
        try {
            super.destroyMushroomCap(level, cutPos, species, tool, endPoints, destroyedCaps, drops);
        } finally {
            protectedCaps.forEach((pos, state) -> level.setBlock(pos, state, Block.UPDATE_CLIENTS));
        }
    }

    private Map<BlockPos, BlockState> hideStandingCapOverlaps(final Level level, final Species species,
